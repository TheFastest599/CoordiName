...
def decimal_to_dms(decimal_degree: float, is_latitude: bool = True) -> tuple:
    ...
    seconds = int(round((decimal_degree - degrees - minutes / 60) * 3600))
    if is_latitude:
...
def coordiname_to_coordinate(word: str) -> dict:
    ...
    try:
        ...
        if None in (lat_degrees_index, lat_min_sec_index, lon_degrees_index, lon_min_sec_index):
            missing_keys = [words[i] for i, v in enumerate(
                [lat_degrees_index, lat_min_sec_index, lon_degrees_index, lon_min_sec_index]) if v is None]
            raise KeyError(
        ...
        # Handle the KeyError specifically
        print(f"Error: {e}")
        return None
